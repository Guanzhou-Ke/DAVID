...
from .augment import Augment, Cutout, RGB2YUV, RGB2Lab, RGB2YCbCr, RGB2LUV
...
def get_train_transformations(args, task='pretext'):
    ...
    if task == 'standard':
        ...
        return transforms.Compose([
            transforms.RandomResizedCrop(args['training_augmentation']['crop_size']),
...
# TODO: Deepfake数据集，一个伪造技术的不同video的frames作为视图。那这样就可以满足一致性是这个方法的一致创作模式，特异性就是对于不同的 \
# 数据的特异创作点。
...
def get_train_dataset(args, transform, neighbors=False, topk_neighbors_train_path=None):
    ...
    if neighbors:
        ...
        train_set = NeighborsDataset(train_set, indices, args.clustering.num_neighbors)

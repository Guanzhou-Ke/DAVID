...
from .consistency_models import ConsistencyEncoder
from .vae import ViewSpecificVAE
from .mi_estimators import CLUBSample


class DAVID(pl.LightningModule):

    def __init__(self, args, consistency_pretrain_weight=None) -> None:
        super().__init__()
        self.args = args
        ...
        if self.args.consis_enc.enable:
            ...
            if consistency_pretrain_weight is not None:
                self.consis_enc.load_pretext_encoder(torch.load(consistency_pretrain_weight, map_location='cpu')['state_dict'])

        if self.args.vspecific.enable:
            # create view-specific encoder.
            for i in range(self.args.views):
                self.__setattr__(f"venc_{i+1}", ViewSpecificVAE(self.args, channels=self.args.consis_enc.in_channel))
            
        # Common feature pooling method. mean, sum, or first
        self.pooling_method = self.args.fusion.pooling_method
        
        self.mi_est = CLUBSample(self.args.disent.x_dim, 
                                 self.args.disent.y_dim, 
                                 hidden_size=self.args.disent.hidden_size)
